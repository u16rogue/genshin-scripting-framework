...
static void set_open_state(bool open)
{
	...
	if (gsf::menu::is_open)
	{
		last_visible = game::sdk::Cursor::get_visible();
		last_lock = game::sdk::Cursor::get_lockState();
	}

	game::sdk::Cursor::set_visible(gsf::menu::is_open ? true : last_visible);
	game::sdk::Cursor::set_lockState(gsf::menu::is_open ? game::sdk::CursorLockMode::None : last_lock);
}
