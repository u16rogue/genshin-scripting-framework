...
namespace utils
{
	template <typename hash_size = std::uint64_t, typename string_t>
	constexpr hash_size hash_fnv1a(const string_t *string)
	{
		static_assert(std::is_same<hash_size, std::uint64_t>::value || std::is_same<hash_size, std::uint32_t>::value, "Invalid hash size type! Only supported types: std::uint64_t and std::uint32_t");
